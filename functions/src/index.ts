...
// Export data population utilities
export { populatePosts } from './populatePosts';
...
/**
 * Cloud Function: Update voteScore when promocode votes change
...
/**
 * Callable Function: Initialize voteScore for existing promocodes
...
export const initializeVoteScores = onCall(async (request) => {
  ...
  try {
    ...
    if (updateCount > 0) {
      await batch.commit();
      logger.info(`Updated ${updateCount} promocodes with voteScore`);
    }

    return {
      success: true,
      message: `Initialized voteScore for ${updateCount} promocodes`,
      updated: updateCount,
...
/**
 * Callable Function: Remove legacy user interaction fields from promocodes
